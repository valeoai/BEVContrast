...
class MinkUNet(nn.Module):
    def __init__(self, **kwargs):
        ...
        out_channels = kwargs.get('out_channels', 0)
        ...
        if out_channels > 0:
            ...
            if 'head' in kwargs and kwargs['head'] == "bn_linear":
                ...
                self.final = nn.Sequential(nn.BatchNorm1d(cs[8], affine=False), nn.Linear(cs[8], out_channels))
            else:
                logging.info("network - linear head")
                self.final = nn.Sequential(nn.Linear(cs[8], out_channels))
        else:
            self.final = None
